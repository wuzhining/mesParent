...
public class BillWarehouseAllocationServiceImpl extends BaseServiceImpl<BillWarehouse> implements BillWarehouseAllocationService {
	...
	private void recursionIn(Long parentId, Long billId, Long billItemId,Long locationId,Long warehouseId, CommonParam commonParam, List<BarcodeMain> listBarcode) {
		...
		try {
			...
			for (BarcodeMain barcodeMain : packBoxlist) {
				...
				barcodeMain.setBarcodeStatusDictId(EnumBarcodeStatus.INSTORE.getValue());
				barcodeMain.setBillStatusDictId(EnumBillStatus.FINISHED.getValue());
				barcodeMain.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue());
	...
	*/
	private List<BarcodeMain> getInsideBarcodeOut(BarcodeMain barcodeMain, Long billId, Long billItemId, Long barcodeStatus,
		...
		// 将当前扫描条码记录
		barcodeMain.setBarcodeStatusDictId(barcodeStatus);
		barcodeMain.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue());
		barcodeMain.setBillId(billId);
		//查询单据号
		BillWarehouse billWarehouse=billWarehouseDao.selectById(billId);
		barcodeMain.setBillCode(billWarehouse.getBillCode());
	...
	*/
			   ...
			   trackBarcode=this.getAllVoByCode(trackBarcodeVo.getBarcode(),trackBarcodeVo.getBillId(),null,null, commonParam);
			     if (trackBarcode !=null) {
			    	 ...
			    	 barCodeList=this.getInsideBarcodeIn(barcodeMainTemp, trackBarcodeVo.getBillId(),
			    			 trackBarcode.getBillItemId(),EnumBarcodeStatus.INSTORE.getValue(),
								EnumBillStatus.FINISHED.getValue(),trackBarcodeVo.getLocationId(),trackBarcodeVo.getWarehouseId(), commonParam);
					...
					//更改当前上架单明细状态为处理中及数量
					billWarehouseItemDao.updateInstore(trackBarcode.getBillItemId(), EnumBillStatus.DOING.getValue(), trackBarcode.getQuantity(), commonParam);
							
					//更新上架单主表状态信息为处理中
					billWarehouseDao.updateBillStatusMain(trackBarcodeVo.getBillId(), EnumBillStatus.DOING.getValue(), commonParam);
					//新增调拨单处理中历史表
					//查询单据号
					BillWarehouse billin=billWarehouseDao.selectById(trackBarcodeVo.getBillId());
					historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.DOING.getValue(), trackBarcodeVo.getBillId(),billin.getBillCode(), commonParam);
				        
					//更新调拨单明细表状态信息为处理中及数量
					billWarehouseDao.updateAllocationInstore(Temp.getBillItemId(), EnumBillStatus.DOING.getValue(), barcodeMainTemp.getQuantity(), commonParam);
				
						
					//判断扫描标签调拨上架单据是否已扫完所有标签，确认扫描完把调拨上架单完结
							TrackBarcode isofList = new TrackBarcode();
							isofList = this.getAllVoByCode(null, trackBarcode.getBillId(), null,EnumBarcodeStatus.NEW.getValue(), commonParam);
							if (isofList == null){
								
							    //更新上架单主表状态信息为完结
								billWarehouseDao.updateBillStatusMain(trackBarcode.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								//新增调拨单处理中历史表
								//查询单据号
								//BillWarehouse bill=billWarehouseDao.selectById(trackBarcodeVo.getBillId());
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.FINISHED.getValue(), trackBarcodeVo.getBillId(),billin.getBillCode(), commonParam);
								//更新上架单主表状态信息为完结
								billWarehouseItemDao.updateBillItemStatus(trackBarcodeVo.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								//更新上架单追踪条码表的单据状态
								trackBarcodeDao.updateTrackStatus(trackBarcodeVo.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							}
							
							
					//判断调拨单明细数量是否已完成所有，若调拨单所有物料全部调拨完成，则完结调拨单
							BillWarehouseItem isofListItem = new BillWarehouseItem();
							isofListItem = this.getbillItemVoBybillId(Temp.getBillId(), null,EnumYesOrNo.YES.getValue(), commonParam);
							if (isofListItem ==null){
								
							    //更新调拨单主表状态信息为完结
								billWarehouseDao.updateBillStatusMain(Temp.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								//更新调拨单明细状态信息为完结
								billWarehouseItemDao.updateBillItemStatus(Temp.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								//更新调拨单追踪条码表的单据状态
								trackBarcodeDao.updateTrackStatus(Temp.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								//新增调拨单处理中历史表
								//查询单据号
								BillWarehouse billAllo=billWarehouseDao.selectById(Temp.getBillId());
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue(), EnumBillStatus.FINISHED.getValue(), Temp.getBillId(),billAllo.getBillCode(), commonParam);
							}
	...
	@Override
	public String  allocationOutput(List<BarcodeMain> list,Long fromWarehouseId,Long  toWarehouseId,Long factory, CommonParam commonParam) throws BusinessException, SQLException{
		...
		for (BarcodeMain item : list) {
			...
			//判断是否在库
			if (BarcodeMainItem.getBarcodeStatusDictId().equals(EnumBarcodeStatus.INSTORE.getValue())) {
				//判断是否为最外层箱号
			   if (BarcodeMainItem.getBarcodePackboxId() ==0) {
				
			
			//更新调拨单主表
			billWarehouseDao.updateBillStatusMain(item.getBillId(),EnumBillStatus.DOING.getValue(),commonParam);
			//新增调拨单处理中历史表
			//查询单据号
			BillWarehouse billAllo=billWarehouseDao.selectById(item.getBillId());
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue(), EnumBillStatus.DOING.getValue(), item.getBillId(),billAllo.getBillCode(), commonParam);
			
			//更新调拨单明细表已扫描数量
			billWarehouseDao.updateAllocationOutstore(item.getBillItemId(), EnumBillStatus.DOING.getValue(), BarcodeMainItem.getQuantity(), commonParam);
			...
			everybarList=this.getInsideBarcodeOut(BarcodeMainItem, item.getBillId(),
					item.getBillItemId(),EnumBarcodeStatus.OFFSTORE.getValue(),
		...
		String billCodeOut = configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(),null, commonParam);
		billWarehouseOut=billWarehouseDao.insertAllocationMainAndDetail(billId,billCodeOut,null,
				      EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(),EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue(),EnumBillStatus.FINISHED.getValue(),fromWarehouseId, toWarehouseId, "OUT",factory, commonParam);
		//新增调拨出库单状态历史表
		historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(), EnumBillStatus.NEW.getValue(), billWarehouseOut.getId(),billCodeOut, commonParam);
		historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(), EnumBillStatus.DOING.getValue(), billWarehouseOut.getId(), billCodeOut,commonParam);
		historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(), EnumBillStatus.FINISHED.getValue(), billWarehouseOut.getId(), billCodeOut,commonParam);
		//查询相应的出库明细Id
		...
		//批量新增调拨出库单条码追踪表
		trackBarcodeDao.insertAllocationInOrOutSnList(billWarehouseOut.getId(), EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue(),EnumBillStatus.FINISHED.getValue(),EnumBarcodeStatus.OFFSTORE.getValue(),allocationTrackList, AbillItemOut, "OUT", commonParam);
		
		
		//调拨上架单
		//生成调拨入库单主表及明细表
		BillWarehouse billWarehouseIn= new BillWarehouse();
		String billCodeIn = configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(),null, commonParam);
		billWarehouseIn=billWarehouseDao.insertAllocationMainAndDetail(billId,billCodeIn,null,
			      EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(),EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue(),EnumBillStatus.NEW.getValue(), fromWarehouseId, toWarehouseId, "IN",factory, commonParam);
		//新增调拨出库单状态历史表
		historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.NEW.getValue(), billWarehouseIn.getId(), billCodeIn,commonParam);		
		//查询相应的入库明细Id
		...
		//批量新增入库上架单条码追踪表
		trackBarcodeDao.insertAllocationInOrOutSnList(billWarehouseIn.getId(), EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(),EnumBillStatus.NEW.getValue(),EnumBarcodeStatus.NEW.getValue(),allocationTrackList, AbillItemIn, "IN", commonParam);
	...
	public ResultMessage barcodeTransposition(BarcodeMain barcodeMain, CommonParam commonParam) throws BusinessException, SQLException{
		...
		if (barcodeMainTemp !=null) {
			...
			//判断此标签是否为上架状态
			if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.INSTORE.getValue())) {
			...
			} else if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.OUTSTORE.getValue())){
				
				throw new BusinessException("该标签已出库！");
				
			}else if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.OFFSTORE.getValue())){
				
				throw new BusinessException("该标签已下架！");
				
			}else if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.NEW.getValue())){
