...
public class WarehouseStrategyDownshelfItemController extends BaseController {
	...
	public ModelAndView edit(HttpServletRequest request, Long id,Long strategyId) {
		...
		dictionaryParamVo.setIsValid(EnumYesOrNo.YES.getValue());
		ModelAndView modelAndView = new ModelAndView();
		...
		try {
			...
			areaParamVo.setIsValid(EnumYesOrNo.YES.getValue());
	...
	public ResultMessage del(HttpServletRequest request,Long id) {
		...
		try {
			if (id != null) {
				clientWarehouseStrategyDownshelfItemService.deleteById(id, this.getCommonParam(request));
