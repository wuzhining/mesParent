...
public class ClientBillDeliveryServiceImpl extends BaseClientServiceImpl<BillDelivery>
	...
	public ResultMessage saveOrUpdate(BillDeliveryParamVo billDeliveryParamVo, CommonParam commonParam) {
		...
		try {
			if (billDeliveryParamVo.getId() == null) {// 新增
                ...
                if (billDeliveryVoTemp.getAuditStatusDictId()!=null) {
                	
					historyBillStatusAuditService.billAuditHistoryInsert(EnumBillType.BILL_TYPE_DELIVERY.getValue(), billDeliveryVoTemp.getAuditStatusDictId(), billDeliveryVoTemp.getId(),billDeliveryVoTemp.getBillCode(), commonParam);
				}
				resultMessage.setIsSuccess(true);
			}
	...
	public ResultMessage purchaseBatchSaveOrUpdate(List<BillPurchaseItemVo> list, CommonParam commonParam) {
		...
		try {
			for (BillPurchaseItemVo bill : list) {
				if ( bill.getQuantityNeed() == null) {
					resultMessage.setMessage("送货数量不能为空");
					return resultMessage;
				}
				if ( bill.getQuantityNeed() <= 0) {
					resultMessage.setMessage("送货数量不能小于或等于0");
					return resultMessage;
				}
			}
			
			billWarehouseDeliveryService.purchaseBatchSaveOrUpdate(list, commonParam);
			resultMessage.setIsSuccess(true);
