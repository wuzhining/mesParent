...
public class BillWarehouseServiceImpl extends BaseServiceImpl<BillWarehouse> implements BillWarehouseService {
	...
	private BillWarehouse insertEntityAndItem(BillWarehouseParamVo paramVo, CommonParam commonParam)
		...
		List<BillWarehouseItem> itemList = billWarehouseItemDao.buildLocationsByRack(paramVo, billWarehouseId, commonParam);
	...
	public void insertBatchEntityAndItem(List<BarcodeMain> totalBarcodeList, Long fromBillId, CommonParam commonParam)
		...
		for (Entry<Long, List<BarcodeMain>> m : barcodeMap.entrySet()) {
			...
			bill.setFromBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_PICKING.getValue());// 来源单据类型：
	...
	public void insertUnPickBatch(BillWarehouseVo billWarehouseVo, List<ProductMaterial> list, CommonParam commonParam)
		...
		String warehouseId = configCodeRuleDao.createCode(BillWarehouse.class.getName(),
				EnumBillType.BILL_TYPE_WAREHOUSE_UNPICKING.getValue(), null, commonParam);
		paramVo.setBillCode(warehouseId);
		paramVo.setTypeStock("1");// 0入库 1出库
		paramVo.setTenantId(commonParam.getTenantId());
		paramVo.setAuditStatusDictId(EnumAuditStatus.UNAUDITED.getValue());
	...
	public void insertShipBatch(BillWarehouseVo billWarehouseVo, List<BillSaleorderItem> list, CommonParam commonParam)
		...
		// 自动生成单据编码
		String warehouseId = configCodeRuleDao.createCode(BillWarehouse.class.getName(),
				EnumBillType.BILL_TYPE_WAREHOUSE_SHIPPING.getValue(), null, commonParam);

		BillSaleorder billSaleorder = billSaleorderDao.selectById(list.get(0).getBillSaleorderId());
		if (billSaleorder != null) {
			paramVo.setFromBillId(billSaleorder.getId());
			paramVo.setFromBillCode(billSaleorder.getBillCode());
		}
		paramVo.setBillCode(warehouseId);
		paramVo.setTypeStock("1");// 0入库 1出库
		paramVo.setTenantId(commonParam.getTenantId());
	...
	public void insertPickTimeTask() throws BusinessException, SQLException {
		...
		workParamVo.setPlanStatusDictId(EnumWorkPlanStatus.NOTSTARTED.getValue());
		...
		for (WorkPlanMaterial workPlanMaterial : wMaterials) {
			...
			bill.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_PICKING.getValue());// 单据类型：
			bill.setBillStatusDictId(EnumBillStatus.NEW.getValue());// 单据状态：
			billList.add(bill);
			BillWarehouseItem billWarehouseItem = new BillWarehouseItem();
			Long itemId = billWarehouseItemDao.getIdValue();
			billWarehouseItem.setId(itemId);
			billWarehouseItem.setBillId(bill.getId());// 单据主表id
			billWarehouseItem.setBillStatusDictId(EnumBillStatus.NEW.getValue());// 单据状态
	...
	@Override
	public void insertPickTkFromWorkId(Long billWorkId, CommonParam commonParam)
			throws BusinessException, SQLException {
		// 根据传入的id查询出该工单详细信息
		BillWorkorder billWorkorder = BillWorkorderDao.selectById(billWorkId);
		// 根据bom id 找到对应bom明细
		List<ProductBom> bomList = productBomDao.selectListBomTree(billWorkorder.getBomId(), commonParam);
		...
		billWarehouseParamVo.setTypeStock(EnumTypeStock.OUT.getValue());// 0入库 1出库
		billWarehouseParamVo.setTenantId(commonParam.getTenantId());
		billWarehouseParamVo.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_PICKING.getValue());//单据类型 生产领料单
		billWarehouseParamVo.setAuditStatusDictId(EnumAuditStatus.UNAUDITED.getValue());//审核状态 未审核
		billWarehouseParamVo.setFromBillTypeDictId(EnumBillType.BILL_TYPE_WORKORDER.getValue());//来源单据类型 工单
		billWarehouseParamVo.setFromBillId(billWorkId);//来源单据id 工单id
		billWarehouseParamVo.setFromBillCode(billWorkorder.getWorkorderCode());//来源单据编码
		
		// 单据明细对象list
		List<BillWarehouseItem> itemList = new ArrayList<>();
		//遍历产品BOM集合 装载领料明细list对象
		for (ProductBom productBom : bomList) {
			...
			billWarehouseItem.setQuantity(productBom.getQuantity());
			billWarehouseItem.setQuantityFinish(0.00);
			billWarehouseItem.setQuantityScan(0.00);
			itemList.add(billWarehouseItem);
		 
		}
		billWarehouseParamVo.setItemList(itemList);
		BillWarehouse  billWarehouse = this.insertEntityAndItem(billWarehouseParamVo, commonParam);
