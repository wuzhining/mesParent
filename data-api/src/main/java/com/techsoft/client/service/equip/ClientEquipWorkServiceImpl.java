...
public class ClientEquipWorkServiceImpl extends BaseClientServiceImpl<EquipWork> implements ClientEquipWorkService {
	...
	private EquipWorkVo getVo(EquipWork equipWork, CommonParam commonParam) throws BusinessException, SQLException {
		...
		//开关状态
		if(vo.getSwitchStatusDictId() != null && vo.getSwitchStatusDictId() > 0L){
			ConfigDictionary switchStatus = configDictionaryService.selectById(vo.getSwitchStatusDictId(), commonParam);
			if(switchStatus != null){
				vo.setSwitchStatus(switchStatus);
			}
		}
	...
	@Override
	public EquipWorkVo getByEquipCode(String equipCode, CommonParam commonParam)
			throws BusinessException, SQLException {
		EquipWork equipWork = equipWorkService.getByEquipCode(equipCode, commonParam);
		if(equipWork!=null)
			return this.getVo(equipWork, commonParam);
