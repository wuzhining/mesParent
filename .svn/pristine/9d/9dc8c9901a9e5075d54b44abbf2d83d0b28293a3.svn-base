...
public class ClientBillWarehouseServiceImpl extends BaseClientServiceImpl<BillWarehouse>
	...
	private BillWarehouseVo getVo(BillWarehouse billWarehouse, CommonParam commonParam)
		...
		if (vo.getAuditStatusDictId() != null && vo.getAuditStatusDictId() > 0L) {
			...
			if (auditStatus != null) {
				vo.setAuditStatusDic(auditStatus);
			}
		}
		if (vo.getFromBillTypeDictId() != null && vo.getFromBillTypeDictId() > 0L) {
			...
			if (fromBillType != null) {
				...
				//单据
				if (vo.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_DELIVERY.getValue())) {//发货单
				...
				} else if (vo.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_SALEORDER.getValue())) {//销售单
				...
				} else if (vo.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_EXAMINE.getValue())) {//检验单
		        ...
		        } else if (vo.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_WORKORDER.getValue())){//工单
		        ...
		        }else if (vo.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_INVENTORY.getValue())){//盘点单
...
//		// ---------------备料看板------------------
	...
	public ResultMessage saveOrUpdate(BillWarehouseParamVo billWarehouseParamVo, CommonParam commonParam) {
		...
		try {
			if (billWarehouseParamVo.getId() == null) {// 新增
				...
				//新增历史表
				historyBillStatusService.billHistoryInsert(billWarehouseParamVo.getBillTypeDictId(), EnumBillStatus.NEW.getValue(), billWarehouse.getId(),createCode, commonParam);
				historyBillStatusAuditService.billAuditHistoryInsert(billWarehouseParamVo.getBillTypeDictId(), billWarehouse.getAuditStatusDictId() , billWarehouse.getId(), billWarehouse.getBillCode(), commonParam);
				...
				//如果更改的是单据审核  增加历史审核单据表
				if (billWarehouseParamVo.getAuditStatusDictId() != null) {
					historyBillStatusAuditService.billAuditHistoryInsert(billWarehouseParamVo.getBillTypeDictId(), billWarehouseParamVo.getAuditStatusDictId() , billWarehouse.getId(), billWarehouse.getBillCode(), commonParam);
				}
	...
	public ResultMessage allocationOutput(List<BarcodeMain> list, Long fromWarehouseId, Long toWarehouseId,Long factoryId,
		...
		try {
			// 向调拨服务请求
			billcode = billWarehouseAllocationService.allocationOutput(list, fromWarehouseId, toWarehouseId,factoryId,
					commonParam);
	...
	public ResultMessage deliverySave(List<BillDelivery> billDelivery,Long factoryId, CommonParam commonParam) {
		ResultMessage resultMessage = new ResultMessage();
		try {
			resultMessage = billWarehouseDeliveryService.deliverySave(billDelivery,factoryId, commonParam);
	...
	@Override
	public ResultMessage discardedOutInsert(List<BarcodeMain> barcodeList,Long factoryId, CommonParam commonParam)
			throws BusinessException, SQLException {
		ResultMessage resultMessage = new ResultMessage();
			resultMessage = billWarehouseDiscardedService.insertDiscarded(barcodeList,factoryId, commonParam);
		return resultMessage;
	}
