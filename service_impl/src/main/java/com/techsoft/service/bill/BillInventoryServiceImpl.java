...
public class BillInventoryServiceImpl extends BaseServiceImpl<BillInventory> implements BillInventoryService {
	...
	public ResultMessage settle(Long id, CommonParam commonParam) throws BusinessException, SQLException {
		...
		paramVoOut.setBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		paramVoOut.setBillId(id);
		paramVoOut.setBillStatusDictId(EnumBillStatus.NEW.getValue());
		settleOut=trackBarcodeDao.selectListByParamVo(paramVoOut);
		
		//根据盘点单得到盘点入库的所有标签
		TrackBarcodeParamVo paramVoIn=new TrackBarcodeParamVo();
		paramVoIn.setBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		paramVoIn.setBillId(id);
		paramVoIn.setBillStatusDictId(EnumBillStatus.FINISHED.getValue());
		...
		paramVoUpdate.setBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		paramVoUpdate.setBillId(id);
		paramVoUpdate.setBillStatusDictId(EnumBillStatus.FINISHED.getValue());
		...
		if(settleOut.size()>0){
		    ...
		    billWarehouseOut.setBillStatusDictId(EnumBillStatus.FINISHED.getValue());
		    billWarehouseOut.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue());
		    billWarehouseOut.setFromBillId(id);
		    billWarehouseOut.setFromBillCode(bill.getBillCode());
		    billWarehouseOut.setFromBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		 ...
		 // 根据物料分组
		    ...
		    for (TrackBarcode trackBarcode : settleOut) {
		    	...
		    	barcodeMain.setBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		    	barcodeMain.setBarcodeStatusDictId(EnumBarcodeStatus.OUTSTORE.getValue());
		    	barcodeMain.setBillId(id);
		    	
		    	//查询单据号
				BillInventory billInventory=billInventoryDao.selectById(id);
				barcodeMain.setBillCode(billInventory.getBillCode());
		    	...
		    	trackBarcode.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_OUT.getValue());
		...
		if(settleIn.size()>0){
           String billCodeIn=configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), null,commonParam);
		    ...
		    billWarehouseIn.setBillStatusDictId(EnumBillStatus.FINISHED.getValue());
		    billWarehouseIn.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue());
		    billWarehouseIn.setFromBillId(id);
		    billWarehouseIn.setFromBillCode(bill.getBillCode());
		    billWarehouseIn.setFromBillTypeDictId(EnumBillType.BILL_TYPE_INVENTORY.getValue());
		 ...
		 // 根据物料分组
		    ...
		    for (TrackBarcode trackBarcode : settleIn) {
		    	...
		    	idString = df.format(index);
 				idLong = Long.valueOf(tracknum.toString() + idString);
		    	trackBarcode.setId(idLong);
		    	trackBarcode.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue());
