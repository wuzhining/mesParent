...
public class Arith {
	...
	public static Integer rintInteger(Double v1) {

		return new BigDecimal(v1).setScale(0, BigDecimal.ROUND_HALF_UP)
				.intValue();
	}
	...
	/**
     * 将double精度的数据,扩大i倍后，按无小数位向上取整转成Long类型数据
     * 
     * @param d
     *            需要转换的数据
     * @param i
     ...
     * @return Long类型数据
     */
    public static Long computeWithDoubleInteger(Double d, Integer i) {
        BigDecimal result = new BigDecimal(Double.toString(d));
        result = result.multiply(new BigDecimal(Integer.toString(i)));
