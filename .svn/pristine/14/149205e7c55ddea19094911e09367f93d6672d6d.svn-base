	...
	public TrackBarcode onBarcode(TrackBarcodeVo trackBarcodeVo, CommonParam commonParam) throws BusinessException, SQLException  {
		...
		TrackBarcode returnBarcode = new TrackBarcode();
			...
			if (barcodeMainTemp !=null) {
				...
				if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.NEW.getValue()) ) {
					...
					if (trackBarcode != null) {
						...
						if (productMain.getStrategyUpshelfId() == null || productMain.getStrategyUpshelfId().equals("")) {
							throw new BusinessException("该物料对应的物品没有分配上架策略！");
						}
						// 调用上架策略接口实现类
						...
						//更改当前入库上架单主表为处理中状态
						billWarehouseDao.updateBillStatusMain(trackBarcode.getBillId(), EnumBillStatus.DOING.getValue(), commonParam);
						
						//新增入库单处理中状态的历史表
						//查询单据号
						BillWarehouse bill=billWarehouseDao.selectById(trackBarcode.getBillId());
						historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.DOING.getValue(), trackBarcode.getBillId(), bill.getBillCode(),commonParam);
						...
						trackisofparamVo.setBillStatusDictId(EnumBillStatus.NEW.getValue());
						...
						if (isofList.size() == 0){
							  ...
							  //更新明细单据状态
								billWarehouseItemDao.updateBillItemStatus(trackBarcode.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //更新追踪条码表的单据状态
								trackBarcodeDao.updateTrackStatus(trackBarcode.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //新增入库单处理中状态的历史表
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.FINISHED.getValue(), trackBarcode.getBillId(), bill.getBillCode(),commonParam);
						}
						...
						returnBarcode=ListVo.get(0);
					    	 ...
					    	 throw new BusinessException("该物品上架策略工厂名称与扫描位置工厂不匹配！");
					     }else if(code==2){
					    	 throw new BusinessException("该物品上架策略所属仓库与扫描位置仓库不匹配！");
					     }else if(code==3){
					    	 throw new BusinessException("该物品上架策略所属区域与扫描位置区域不匹配！");
					     }else if(code==4){
					    	 throw new BusinessException("该物品上架策略所属货架与扫描位置货架不匹配！");
					     }else if(code==5){
					    	 throw new BusinessException("该物品上架策略所属货位与扫描位置货位不匹配！");
					     }else if(code==6){
					    	 throw new BusinessException("该物品上架策略库位必须是空！");
					     }else if(code==7){
					    	 throw new BusinessException("该物品上架策略库位不允许混放批次！");
					     }else if(code==8){
					    	 throw new BusinessException("该物品上架策略库位不允许混放物品！");
					     }else if(code==9){
					    	 throw new BusinessException("该物品上架策略库位必须有相同批次！");
					     }else if(code==10){
					    	 throw new BusinessException("该物品上架策略库位必须有相同物品！");
					     }
				 }  else {
	...
	public TrackBarcode billReturnInput(TrackBarcode track, CommonParam commonParam) throws BusinessException, SQLException  {
		...
		TrackBarcode returnBarcode = new TrackBarcode();
			...
			if (barcodeMainTemp !=null) {
				...
				//判断标签状态是否为新建状态，只有新建的标签才能上架，如果不是则返回相应状态
				if (barcodeMainTemp.getBarcodeStatusDictId().equals(EnumBarcodeStatus.OUTSTORE.getValue()) ) {
					...
					if (trackBarcode != null) {
						...
						barCodeList = this.getInsideBarcode(barcodeMainTemp, track.getBillId(),
								track.getBillItemId(),EnumBarcodeStatus.INSTORE.getValue(),
						EnumBillStatus.FINISHED.getValue(),track.getLocationId(),track.getWarehouseId(), commonParam);
						...
						//更改当前入库上架单明细表为处理中状态
						billWarehouseItemDao.updateInstore(track.getBillItemId(), EnumBillStatus.DOING.getValue(), trackBarcode.getQuantity(), commonParam);
							
						//更改当前入库上架单主表为处理中状态
						billWarehouseDao.updateBillStatusMain(track.getBillId(), EnumBillStatus.DOING.getValue(), commonParam);
						
						//新增退料入库单处理中状态的历史表
						BillWarehouse billWarehouse=billWarehouseDao.selectById(track.getBillId());
						historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.DOING.getValue(), track.getBillId(),billWarehouse.getBillCode(), commonParam);
						...
						if (isofList.size() == 0){
							  //更新退料上架单主表状态信息为完结
								billWarehouseDao.updateBillStatusMain(track.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //更新退料上架单明细单据状态
								billWarehouseItemDao.updateBillItemStatus(track.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //更新退料上架单追踪条码表的单据状态
								trackBarcodeDao.updateTrackStatus(track.getBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
								
							  //新增退料入库单完结状态的历史表
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.FINISHED.getValue(), track.getBillId(),billWarehouse.getBillCode(), commonParam);
								
							  //查询退料单主单
								BillWarehouse bill = billWarehouseDao.selectById(track.getBillId());
							  //更新退料单主表状态信息为完结
								billWarehouseDao.updateBillStatusMain(bill.getFromBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //更新退料单明细单据状态
								billWarehouseItemDao.updateBillItemStatus(bill.getFromBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //更新退料单追踪条码表的单据状态
								trackBarcodeDao.updateTrackStatus(bill.getFromBillId(), EnumBillStatus.FINISHED.getValue(), commonParam);
							  //新增退料单完结状态的历史表
								BillWarehouse billReturn = billWarehouseDao.selectById(bill.getFromBillId());
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue(), EnumBillStatus.FINISHED.getValue(), bill.getFromBillId(), billReturn.getBillCode(),commonParam);
						}
					...
					if (trackBarcode != null) {
						...
						if (isofList.size() == 0){
							  ...
							  //新增退料单完结状态的历史表
								...
								historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_REJECT_CUSTOMER.getValue(), EnumBillStatus.FINISHED.getValue(), bill.getFromBillId(),billReject.getBillCode(), commonParam);
						}
