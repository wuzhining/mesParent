	...
	public ResultMessage saveOrUpdate(ProductPackboxParamVo productPackboxParamVo, CommonParam commonParam) {
		...
		try {
			if (productPackboxParamVo.getId() == null) {// 新增
				...
				temp.setMaterialId(productPackboxParamVo.getMaterialId());
				List<ProductPackbox> tempList=productPackboxService.selectListByParamVo(temp, commonParam);
				if (tempList.size()>0) {
					resultMessage.addErr("物品包装编码已经包装！");
					return resultMessage;
				}
				//生成编码
				String code = productPackboxParamVo.getPackboxCode();
				if(StringUtils.isBlank(code)){
					code = configCodeRuleService.createCode(ProductPackbox.class.getName(), null, null,commonParam);
					productPackboxParamVo.setPackboxCode(code);
				}
				if(StringUtils.isBlank(code)){
					resultMessage.addErr("物品包装编码不能为空");
					return resultMessage;
				}
				//检验编码是否已存在
				ProductPackboxParamVo packboxParamVoSearch = new ProductPackboxParamVo();
				packboxParamVoSearch.setPackboxCode(code);
				List<ProductPackbox> list = productPackboxService.selectListByParamVo(packboxParamVoSearch, commonParam);
				if((list!=null)&&(!list.isEmpty())){
					resultMessage.addErr("物品规格编码已存在");
					return resultMessage;
				}
				productPackboxParamVo.setPackboxCode(code);
				productPackboxParamVo.setTenantId(commonParam.getTenantId());
