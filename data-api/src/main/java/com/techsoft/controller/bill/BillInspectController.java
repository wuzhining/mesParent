...
public class BillInspectController extends BaseController {
	...
	public ReturnPageInfo listJson(HttpServletRequest request,
		...
		try {
            ...
            if (!"".equals(billInspectParamVo.getFromBillIdLike()) && billInspectParamVo.getFromBillIdLike() != null ) {
            	BillWarehouseParamVo billWarehouseParamVo = new BillWarehouseParamVo();
            	billWarehouseParamVo.setBillCodeLike(billInspectParamVo.getFromBillIdLike());
    			List<BillWarehouse> billWarehouseList = clientBillWarehouseService.selectListByParamVo(billWarehouseParamVo,this.getCommonParam(null));
    			if(billWarehouseList.size()> 0){
	    			List<Long> list = new ArrayList<Long>();
	    			for (BillWarehouse billWarehouse : billWarehouseList) {    				
	    				list.add(billWarehouse.getId());	
	    				billInspectParamVo.setFromBillIdlist(list);
					}
    			}
			}
	...
	public ResultMessage verify(HttpServletRequest request,BillInspectParamVo billInspectParamVo) {
		...
		billInspectParamVo.setModifyUserId(getLoginUserId(request));
		billInspectParamVo.setInspectStatusDictId(EnumBillStatus.DOING.getValue());
