...
public class BillSaleorderServiceImpl extends BaseServiceImpl<BillSaleorder> implements BillSaleorderService {
	...
	protected BillSaleorder insertEntity(BillSaleorder entity, CommonParam commonParam) throws BusinessException, SQLException {
		...
		if(insertEntity!=null){
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_SALEORDER.getValue(),insertEntity.getBillStatusDictId(), insertEntity.getId(),insertEntity.getBillCode(),commonParam);
			//增加单据审批记录
			historyBillStatusAuditDao.billHistoryInsert(EnumBillType.BILL_TYPE_SALEORDER.getValue(),insertEntity.getAuditStatusDictId(), insertEntity.getId(),insertEntity.getBillCode(), commonParam);
		}
		//增加单据历史记录
		return insertEntity;
	}
	...
	protected BillSaleorder updatePartEntity(BillSaleorder entity, CommonParam commonParam) throws BusinessException, SQLException {
		...
		if(updatePartEntity!=null){
			//增加单据历史记录
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_SALEORDER.getValue(), updatePartEntity.getBillStatusDictId(), updatePartEntity.getId(),updatePartEntity.getBillCode(), commonParam);
			//增加单据审批记录,审批id不为未审批状态时，表示已修改审批状态
			if(updatePartEntity.getAuditStatusDictId()!=EnumAuditStatus.UNAUDITED.getValue()){
				historyBillStatusAuditDao.billHistoryInsert(EnumBillType.BILL_TYPE_SALEORDER.getValue(),updatePartEntity.getAuditStatusDictId(), updatePartEntity.getId(),updatePartEntity.getBillCode(), commonParam);
			}
			//更新明细表
