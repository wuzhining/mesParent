...
public class ClientUserPassportServiceImpl extends BaseClientServiceImpl<UserPassport>
	...
	public ResultMessage loginByUserName(CommonParam commonParam, String userName, String password, String loginip) {
		...
		if (StringUtils.isBlank(userName)) {
			resultMessage.addErr("用户名或工号不能为空");
		...
		try {
			ConfigParameter configParameter = configParameterService.getByCode(ConstantParameter.CURRENT_TENANT_ID,
					commonParam);

			if (userName.contains("_")) {

				UserTenant userTenant = userTenantService.selectById(new Long(configParameter.getParamValue()),
						commonParam);

				UserPassport userTenantPassport = userPassportService.selectById(userTenant.getUserId(), commonParam);
				userName = userTenantPassport.getUserName() + "_" + userName;
	...
	public ResultMessage saveOrUpdate(UserPassportParamVo userPassportParamVo, CommonParam commonParam) {
		...
		try {
			if (userPassportParamVo.getId() == null) {// 新增
				...
				FirmEmployee firmEmployee = firmEmployeeService.getByUserId(userPassportParamVo.getId());
				if (firmEmployee != null) {
					firmEmployee.setRealName(userPassportVoTemp.getRealName());
					firmEmployee = firmEmployeeService.saveOrUpdate(firmEmployee, commonParam);
				}
	...
	public ResultMessage modifiyUserPass(UserPassportVo userPassportVo, String oldPassword, String newPassword,
		...
		try {
			if (oldPassword == null) {
				...
				UserPassport userPassportTemp=new UserPassport();
				// 查询用户
				if (userPassportVo.getUserName() == null) {
					userPassportVo.setId(Long.parseLong(commonParam.getUserId()));
					userPassportTemp = userPassportService.selectById(Long.parseLong(commonParam.getUserId()), commonParam);
				}else{
				 userPassportTemp = userPassportService.getByUserName(userPassportVo.getUserName());
				}
				if (userPassportTemp != null) {
					// 验证旧密码是否正确
