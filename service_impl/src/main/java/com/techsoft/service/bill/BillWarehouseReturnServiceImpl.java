...
public class BillWarehouseReturnServiceImpl extends BaseServiceImpl<BillWarehouse> implements BillWarehouseReturnService {
	...
	@Resource
	protected ProductMaterialDao productMaterialDao;
	...
	public ResultMessage workerReturnSava(List<BillWorkorder> list, CommonParam commonParam)
			...
			String billCode = configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue(), null,commonParam);
			...
			for (BillWorkorder billWorkorder : list) {
				...
				billWarehouse.setBillStatusDictId(EnumBillStatus.NEW.getValue());
				billWarehouse.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue());
				billWarehouse.setFromBillId(billWorkorder.getId());
				billWarehouse.setFromBillTypeDictId(EnumBillType.BILL_TYPE_WORKORDER.getValue());
	...
	public ResultMessage barcodeReturn(List<BarcodeMain> list, CommonParam commonParam)
			...
			String billReturnCode = configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue(), null,commonParam);
			String billInputCode = configCodeRuleDao.createCode(BillWarehouse.class.getName(),EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), null,commonParam);
			...
			billReturn.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue());
			billReturn.setBillStatusDictId(EnumBillStatus.DOING.getValue());
			...
			//新增退料单新建、处理中状态历史表
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue(), EnumBillStatus.NEW.getValue(), billRet.getId(), billReturnCode,commonParam);
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue(), EnumBillStatus.DOING.getValue(), billRet.getId(),billReturnCode, commonParam);
			
			//生成退料上架单主表信息
			BillWarehouse billInput= new BillWarehouse();
			billInput.setBillCode(billInputCode);
			billInput.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue());
			billInput.setBillStatusDictId(EnumBillStatus.NEW.getValue());
			...
			//新增退料上架单新建状态历史表
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue(), EnumBillStatus.NEW.getValue(), billIn.getId(), billInputCode,commonParam);
			...
			for (Entry<Long, List<BarcodeMain>> m : barcodeMap.entrySet()) {
				...
				//查询规格成品
				ProductMaterial material=productMaterialDao.selectById(matlist.get(0).getMaterialId());
				billWarehouseItem.setProductId(material.getProductId());
			...
			for (BillWarehouseItem inputList : itemReturnList) {
				...
				billInputItem.setBillStatusDictId(EnumBillStatus.NEW.getValue());
				billInputItem.setQuantity(inputList.getQuantity());
				billInputItem.setMaterialId(inputList.getMaterialId());
				billInputItem.setProductId(inputList.getProductId());
			...
			for (BarcodeMain barcodeMain : barcodeList) {
				...
				trackBarcodeItem.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue());
			...
			for (BarcodeMain barcodeMain : barcodeList) {
				...
				trackBarcodeInItem.setBarcodeStatusDictId(EnumBarcodeStatus.NEW.getValue());
				...
				trackBarcodeInItem.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_IN.getValue());
				...
				trackBarcodeInItem.setBillStatusDictId(EnumBillStatus.NEW.getValue());
