...
public class ClientBillWorkorderServiceImpl extends BaseClientServiceImpl<BillWorkorder>
	...
	private BillWorkorderVo getVo(BillWorkorder billWorkorder, CommonParam commonParam)
		...
		// 工单审批状态
		if (vo.getAuditStatusDictId() != null && vo.getAuditStatusDictId() > 0L) {
			ConfigDictionary auditStatusDict = configDictionaryService.selectById(vo.getAuditStatusDictId(),
					commonParam);
	...
	public ResultMessage saveOrUpdate(BillWorkorderParamVo billWorkorderParamVo, CommonParam commonParam) {
		...
		/*
		 * if (billWorkorderParamVo.getWorkshopId() == null ||
		 * billWorkorderParamVo.getWorkshopId().equals(0L)) {
		 * resultMessage.addErr("车间不能为空 "); return resultMessage; } if
		 * (billWorkorderParamVo.getMaterialId() == null ||
		 * billWorkorderParamVo.getMaterialId().equals(0L)) {
		 * resultMessage.addErr("物品规格不能为空 "); return resultMessage; } if
		 * (billWorkorderParamVo.getProductId() == null ||
		 * billWorkorderParamVo.getProductId().equals(0L)) {
		 * resultMessage.addErr("物品不能为空 "); return resultMessage; }
		 * 
		 * if (billWorkorderParamVo.getWorkorderStatusDictId() == null ||
		 * billWorkorderParamVo.getWorkorderStatusDictId().equals(0L)) {
		 * resultMessage.addErr("工单状态不能为空 "); return resultMessage; } if
		 * (billWorkorderParamVo.getQuantity() == null ||
		 * billWorkorderParamVo.getQuantity() < 1) {
		 * resultMessage.addErr("工单数量不能为0"); return resultMessage; } if
		 * (billWorkorderParamVo.getQuantityMaximumInput() == null ||
		 * billWorkorderParamVo.getQuantityMaximumInput() < 1) {
		 * resultMessage.addErr("最大入库数量不能为0"); return resultMessage; } if
		 * (billWorkorderParamVo.getRatioAllowover() == null) {
		 * resultMessage.addErr("允超比率不能为空"); return resultMessage; } if
		 * (billWorkorderParamVo.getTimePlanStart() == null) {
		 * resultMessage.addErr("计划开始时间不能为空"); return resultMessage; } if
		 * (billWorkorderParamVo.getTimePlanEnd() == null) {
		 * resultMessage.addErr("计划结束时间不能为空"); return resultMessage; }
		 */
		...
		try {
			if (billWorkorderParamVo.getId() == null) {// 新增
				// 自动生成工单编码
				String workordercode = configCodeRuleService.createCode(BillWorkorder.class.getName(), null, null,
						commonParam);
				...
				// 调用批量修改工单bom
				billWorkorderService.saveOrUpdate(billWorkorderVoTemp, commonParam);
				resultMessage.setIsSuccess(true);
			}
	...
	public ResultMessage updateAudit(Long id, CommonParam commonParam) {
		...
		ResultMessage resultMessage = new ResultMessage();
		resultMessage = billWorkorderService.updateAudit(id, commonParam);
		return resultMessage;
	}

	@Override
	public PageInfo<ProductBomVo> listProductBomProcessPage(Long id,
			CommonParam commonParam, int pageNo, int pageSize) throws BusinessException, SQLException {
		 
		PageInfo<ProductBomVo> list = new PageInfo<>();
		List<ProductBomVo> bomlist= billWorkorderService.listProductBomProcess(id, commonParam);
		list.setList(bomlist);
	...
	@Override
	public ResultMessage updatStart(Long id, List<StructProductionlineParamVo> productionlineList,
			CommonParam commonParam) {
		ResultMessage resultMessage = new ResultMessage();
		resultMessage = billWorkorderService.updateStart(id, productionlineList, commonParam);
		return resultMessage;
	}
	...
	public ResultMessage printBarcode(Long id, CommonParam commonParam) throws BusinessException, SQLException {
		ResultMessage resultMessage = new ResultMessage();
		resultMessage = billWorkorderService.insertPrintBarcode(id, commonParam);
		return resultMessage;
	}
}
