...
public class TrackBarcodeDaoImpl extends BaseDaoImpl<TrackBarcode> implements TrackBarcodeDao {
	...
	public void insertSnlist(TrackBarcodeParamVo paramVo, CommonParam commonParam) throws BusinessException, SQLException{
		...
		for (TrackBarcode itemSn : listsns) {
			...
			if (itemSn.getBarcode() == null) {
				itemSn.setBarcode(itemSn.getId().toString());
			}
		}
	...
	public List<TrackBarcode> insertAllocationSnList( List<BarcodeMain> list, CommonParam commonParam)
		...
		for (BarcodeMain trItem : list) {
			...
			try {
				BeanUtil.copyNotNullProperties(trackBarcodeItem, trItem);
			} catch (IllegalAccessException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (InvocationTargetException e) {
			...
			trackBarcodeItem.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_ALLOCATION.getValue());
...
//
	...
	public List<TrackBarcode> insertAllocationInOrOutSnList(Long billId,Long billType,Long billStatus,Long barcodeStatus,List<TrackBarcode> list, List<BillWarehouseItem> itemList,String type,
		...
		for (TrackBarcode allOutBarItem : list) {
			...
			outBarcodeItem.setProductTypeDictId(allOutBarItem.getProductTypeDictId());
			outBarcodeItem.setBarcodeId(allOutBarItem.getBarcodeId());
	...
	*/
	...
	public void updateTrackStatus(Long billId, Long billStatusDictId, CommonParam commonParam)
		...
		//查询标签信息ID更新
				...
				if (list.size() <=0) {
					throw new BusinessException("追踪条码表没有记录！");
				}
	...
	public TrackBarcode insertIqcSnlist(Long arrivalBillId,List<BillInspect> list, CommonParam commonParam)
		...
		for (TrackBarcode iqcSn : arrivallistSn){
			...
			trackBarcode.setBillStatusDictId(EnumBillStatus.NEW.getValue());
			...
			trackBarcode.setBarcodeStatusDictId(EnumBarcodeStatus.NEW.getValue());
			trackBarcode.setBillTypeDictId(EnumBillType.BILL_TYPE_EXAMINE.getValue());
			//取质检明细单
		...
		for (BillDeliveryItem item : itemList) {
		   ...
		   for (TrackBarcode trackBarcode : barcodeItem) {
			   TrackBarcode BarcodeItem = new TrackBarcode();
				...
				BarcodeItem.setBarcodeStatusDictId(EnumBarcodeStatus.NEW.getValue());
				BarcodeItem.setBillStatusDictId(EnumBillStatus.NEW.getValue());
				BarcodeItem.setBillTypeDictId(EnumBillType.BILL_TYPE_DELIVERY_SUPPLIER.getValue());
	...
	public List<TrackBarcode> insertArrivalSnList(List<BillWarehouseItem> itemList,Long deliveryCode,CommonParam commonParam) throws BusinessException, SQLException {
		   ...
		   //批量新增标签
		   for (TrackBarcode trackBarcode : barcodeItem) {
			   TrackBarcode barcodeItems = new TrackBarcode();
				index++;
				idString=df.format(index);
				idLong = Long.valueOf(billItemId.toString() + idString);
				barcodeItems.setId(idLong);
				barcodeItems.setBarcodeStatusDictId(EnumBarcodeStatus.NEW.getValue());
				barcodeItems.setBillStatusDictId(EnumBillStatus.DOING.getValue());
				barcodeItems.setBillTypeDictId(EnumBillType.BILL_TYPE_WAREHOUSE_ARRIVAL.getValue());
				barcodeItems.setBarcode(trackBarcode.getBarcode());
				barcodeItems.setBarcodePackboxId(trackBarcode.getBarcodePackboxId());
				barcodeItems.setBarcodeRuleDictId(trackBarcode.getBarcodeRuleDictId());
				barcodeItems.setProductTypeDictId(trackBarcode.getProductTypeDictId());
				barcodeItems.setBarcodeId(trackBarcode.getBarcodeId());
				barcodeItems.setBillId(itemList.get(0).getBillId());
				//查询相应的明细Id
				for (BillWarehouseItem billItem : itemList) {
					if (billItem.getMaterialId().equals(trackBarcode.getMaterialId())) {
						barcodeItems.setBillItemId(billItem.getId());
					}
				}
				barcodeItems.setSupplierId(trackBarcode.getSupplierId());
				barcodeItems.setBatchNumber(trackBarcode.getBatchNumber());
				barcodeItems.setBatchNumber(trackBarcode.getBatchNumber());
				barcodeItems.setMaterialId(trackBarcode.getMaterialId());
				barcodeItems.setProductId(trackBarcode.getProductId());
				barcodeItems.setQuantity(trackBarcode.getQuantity());
				barcodeItems.setTenantId(commonParam.getTenantId());
				barcodeItems.setCreateUserId(Long.valueOf(commonParam.getUserId()));
				barcodeItems.setModifyUserId(Long.valueOf(commonParam.getUserId()));
				allOutBarList.add(barcodeItems);
		}
