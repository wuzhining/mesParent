...
public class ClientAlarmInforFlowServiceImpl extends BaseClientServiceImpl<AlarmInforFlow>
	...
	private AlarmInforFlowVo getVo(AlarmInforFlow alarmInforFlow, CommonParam commonParam)
		...
		if (vo.getEquipWorkId() != null && vo.getEquipWorkId() > 0L) {
			EquipWork eWork = equipWorkService.selectById(vo.getEquipWorkId(),commonParam);
			if (eWork != null) {
				vo.setEquipWork(eWork);
			}
		}
		...
		if (vo.getDealResultDictId() != null && vo.getDealResultDictId() > 0L) {
			...
			if (dealResult != null) {
				vo.setDealResultDict(dealResult);
			}
		}
		
		
		if (vo.getAlarmUserId() != null && vo.getAlarmUserId() > 0L) {
			UserPassport alarmUser = userPassportService.selectById(vo.getAlarmUserId(), commonParam);
			if (alarmUser != null) {
				vo.setAlarmUser(alarmUser);
			}
		}
