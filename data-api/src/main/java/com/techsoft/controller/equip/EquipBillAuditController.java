...
@Controller
@RequestMapping("/equip/equipBillAudit")
public class EquipBillAuditController extends BaseController {
	@Autowired
	...
	public ModelAndView list(HttpServletRequest request) {
		...
		modelAndView.setViewName("equip/equipBillAudit-list");
		return modelAndView;
	}
	

	@RequestMapping("/edit")
	public ModelAndView edit(HttpServletRequest request, Long id , Long billStatusFixtureDictId) {
		ModelAndView modelAndView = new ModelAndView();
		try {
			//拿到单据状态
			List<ConfigDictionary> billStatusFixtureDict = clientConfigDictionaryService
					.selectListByParentId(EnumDictionary.BILL_STATUS_FIXTURE.getValue());
			List<ConfigDictionary> billStatusFixtureDict1 = new ArrayList<ConfigDictionary>();
			if (billStatusFixtureDictId == 10431 ||billStatusFixtureDictId == 10432||billStatusFixtureDictId ==10433) {//领用
				for (ConfigDictionary configDictionary:billStatusFixtureDict) {
					if (configDictionary.getId() == 10432 || configDictionary.getId() == 10433) {
						billStatusFixtureDict1.add(configDictionary);
					}
				}
			}
			if (billStatusFixtureDictId == 10437||billStatusFixtureDictId == 10438||billStatusFixtureDictId ==10439) {//维修
				for (ConfigDictionary configDictionary:billStatusFixtureDict) {
					if (configDictionary.getId() == 10438 || configDictionary.getId() == 10439) {
						billStatusFixtureDict1.add(configDictionary);
					}
				}
			}
			if (billStatusFixtureDictId == 10445||billStatusFixtureDictId == 10446||billStatusFixtureDictId ==10447) {//报废
				for (ConfigDictionary configDictionary:billStatusFixtureDict) {
					if (configDictionary.getId() == 10446 || configDictionary.getId() == 10447) {
						billStatusFixtureDict1.add(configDictionary);
					}
				}
			}
			modelAndView.addObject("billStatusFixtureDict", billStatusFixtureDict1);
			...
			modelAndView.setViewName("equip/equipBillAudit-edit");
			initData(modelAndView);
		} catch (Exception e) {
			e.printStackTrace();
		}
