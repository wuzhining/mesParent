...
public class PdaReturnController extends BaseController {
			...
			if (barcodeMainList.size() == 0) {
                ...
                if (barcodeMainList.get(0).getBarcodeStatusDictId().equals(EnumBarcodeStatus.OUTSTORE.getValue())) {
                	...
                	if (barcodeMainList.get(0).getBarcodePackboxId()==0){
                		...
                		if (barcodeMainList.get(0).getBillTypeDictId().equals(EnumBillType.BILL_TYPE_WAREHOUSE_PICKING.getValue())){
	...
	*/
	...
	public ResultMessage barcodeReturn(HttpServletRequest request,String barcodeStr,String factoryId) {
		ResultMessage resultMessage = new ResultMessage();
	...
	public ResultMessage barcodeToBill(HttpServletRequest request,String barcode,String factoryId) {
		...
		try {
			...
			if (returnList.size()>0) {
				...
				if (billWarehouse.getFromBillTypeDictId().equals(EnumBillType.BILL_TYPE_WAREHOUSE_MATER_RETURN.getValue())) {
