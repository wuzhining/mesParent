...
public class BillWarehouseOutServiceImpl extends BaseServiceImpl<BillWarehouse> implements BillWarehouseOutService {
	...
	@Override
	public void updateOutBarcode(String billId,Long factoryId,  CommonParam commonParam) throws BusinessException, SQLException {
		...
		// 领料单完结自动生成对应仓库的出库单据
		this.outBill(totalBarcodeList, Long.valueOf(billId), billWarehouse.getBillTypeDictId(), factoryId,commonParam);
		// 单据状态变更历史记录
	 ...
	 */
	private void outBill(List<BarcodeMain> totalBarcodeList, Long fromBillId, Long fromBillType,Long factoryId,
		...
		for (BillWarehouseItem billWarehouseItem : itemList) {
			...
			*/
			...
			WarehouseProductStockParamVo productParamVo = new WarehouseProductStockParamVo();
			//productParamVo.setFactoryId(location.getFactoryId());
			productParamVo.setProductId(billWarehouseItem.getProductId());
			productParamVo.setMaterialId(billWarehouseItem.getMaterialId());
			warehouseProductStockDao.productStockChange(productParamVo, billWarehouseItem.getQuantityScan(), "OUT", commonParam);
		}
	}
