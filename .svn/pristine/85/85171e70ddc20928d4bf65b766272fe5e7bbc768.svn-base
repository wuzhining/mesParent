	...
	public ResultMessage saveOrUpdate(EquipClassesFixtureParamVo equipClassesFixtureParamVo, CommonParam commonParam) { 
		...
		try {
			...
			if (equipClassesFixtureParamVo.getId() == null) {//新增
				//生成工具分类编码
				String code = equipClassesFixtureParamVo.getClassesCode();
				if(StringUtils.isBlank(code)){
					 code = configCodeRuleService.createCode(EquipClassesFixture.class.getName(), null, null, commonParam);
					 equipClassesFixtureParamVo.setClassesCode(code);
				}
				if(StringUtils.isBlank(code)){
					resultMessage.addErr("工具分类编码不能为空");
					return resultMessage;
				}
				//检验编码是否已存在
				EquipClassesFixtureParamVo equipClassesFixtureSearch = new EquipClassesFixtureParamVo();
				equipClassesFixtureSearch.setClassesCode(code);
				List<EquipClassesFixture> list = equipClassesFixtureService.selectListByParamVo(equipClassesFixtureSearch, commonParam);
				if((list!=null)&&(!list.isEmpty())){
					resultMessage.addErr("工具分类编码已存在");
					return resultMessage;
				}
					equipClassesFixtureParamVo.setTenantId(commonParam.getTenantId());
					equipClassesFixture = equipClassesFixtureService.saveOrUpdate(equipClassesFixtureParamVo,commonParam);
