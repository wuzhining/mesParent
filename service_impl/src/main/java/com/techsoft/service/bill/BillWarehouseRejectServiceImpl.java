...
public class BillWarehouseRejectServiceImpl extends BaseServiceImpl<BillWarehouse> implements BillWarehouseRejectService {
	...
	public ResultMessage rejectBatchSaveOrUpdate(List<BillWarehouse> list, CommonParam commonParam) throws BusinessException, SQLException{
		ResultMessage resultMessage = new ResultMessage();
		    String billCode = configCodeRuleDao.createCode(BillDelivery.class.getName(),EnumBillType.BILL_TYPE_DELIVERY_SUPPLIER.getValue(), null,commonParam);
			// 新增主表及明细表
			...
			//新增退货发货单新建状态历史表
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_DELIVERY_SUPPLIER.getValue(), EnumBillStatus.NEW.getValue(), billWarehousItem.get(0).getBillId(),billCode, commonParam);

			for (BillDeliveryItem billDeliveryItem : billWarehousItem) {
				
			// 更新退货单状态
			billWarehouseDao.updateBillStatusMain(billDeliveryItem.getFromBillId(), EnumBillStatus.FINISHED.getValue(),commonParam);
				
			// 更新退货单明细表状态信息为完结
			billWarehouseItemDao.updateBillItemStatus(billDeliveryItem.getFromBillId(),EnumBillStatus.FINISHED.getValue(), commonParam);
				
			// 更新退货单追踪条码表的单据状态
			trackBarcodeDao.updateTrackStatus(billDeliveryItem.getFromBillId(), EnumBillStatus.FINISHED.getValue(),commonParam);
			
			//新增退货发货单新建状态历史表
			BillWarehouse billWarehouse=billWarehouseDao.selectById(billDeliveryItem.getFromBillId());
			historyBillStatusDao.billHistoryInsert(EnumBillType.BILL_TYPE_WAREHOUSE_REJECT_SUPPLER.getValue(), EnumBillStatus.FINISHED.getValue(), billDeliveryItem.getFromBillId(), billWarehouse.getBillCode(),commonParam);
